...
import type { ClassroomSection, ExamSection } from "../../lib/data/classroom";
...
function ClassroomQuestionsView({ theme }: { theme: string }) {
  return (
    <div className="space-y-8">
      {CLASSROOM_QUESTIONS.map((section, idx) => (
...
function ExamsView({ theme }: { theme: string }) {
  return (
    <div className="space-y-8">
      {CLASSROOM_EXAMS.map((exam, idx) => (
        <div key={idx} className={`rounded-lg overflow-hidden border ${
          theme === 'dark' ? "bg-[#0f2438] border-[#1269cc]/30" : "bg-white border-gray-200 shadow-lg"
        }`}>
  ...
  return (
    <div className={`min-h-screen pt-24 pb-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden transition-colors ${theme === 'dark' ? "bg-[#0a1929]" : "bg-gray-50"}`}>
      ...
      <div className="relative z-10 flex gap-2 mb-8">
        ...
        >
          ...
          <span className={`text-xs px-1.5 py-0.5 rounded ${
            ...
            {CLASSROOM_QUESTIONS.reduce((sum, s) => sum + s.items.length, 0)}
          </span>
        </button>
        <button
        ...
        >
          ...
          <span className={`text-xs px-1.5 py-0.5 rounded ${
            ...
            {CLASSROOM_EXAMS.reduce((sum, s) => sum + s.items.length, 0)}
          </span>
        </button>
      </div>
      ...
      <div className="relative z-10">
        {activeTab === "questions" ? (
          <ClassroomQuestionsView theme={theme} />
        ) : (
          <ExamsView theme={theme} />
        )}
      </div>
    </div>
  );
