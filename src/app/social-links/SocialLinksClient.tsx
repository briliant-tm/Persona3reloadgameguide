  ...
  return (
    ...
    >
      ...
      <div className="p-5 space-y-3">
        ...
        <Link
          href={`/social-links/${link.arcana.toLowerCase().replace(/\s+/g, "-")}`}
...
interface SocialLinksClientProps {
    ...
    priorityTiers: { tier: string; label: string; description: string; color: string; iconBg: string; }[];
}

export default function SocialLinksClient({ socialLinks, monthlyGuide, priorityTiers }: SocialLinksClientProps) {
  ...
  }, [search, filter, socialLinks]);

  const essentialLinks = socialLinks.filter(l => l.priority === "essential");
  const recommendedLinks = socialLinks.filter(l => l.priority === "recommended");
  const optionalLinks = socialLinks.filter(l => l.priority === "optional");

  const tabs = [
    { id: "all" as const, label: "All Social Links", icon: <Heart size={16} />, count: socialLinks.length },
    { id: "priority" as const, label: "Priority Guide", icon: <Trophy size={16} />, count: null },
    { id: "guide" as const, label: "Monthly Schedule", icon: <CalendarDays size={16} />, count: monthlyGuide.length },
  ];
        ...
        {activeTab === "all" && (
          ...
          >
            ...
            <p className={`text-sm mb-4 relative z-10 ${theme === "dark" ? "text-gray-500" : "text-gray-400"}`}>
              Showing {filteredLinks.length} of {socialLinks.length} Social Links
            </p>
